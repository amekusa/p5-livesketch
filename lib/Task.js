...
class Task extends Callable {
	...
	resolver(resolve, reject) {
		logger.log(`${this.expr} is ${chalk.yellow('running')}...`);
		let _resolve = arg => {
			logger.log(`${this.expr} has been ${chalk.green('resolved')}`);
	...
	__call() {
		...
		if (this.hasDep) {
			logger.log(`${chalk.yellow('Resolving')} dependencies of ${this.expr}`);
			let promises = [];
			for (let dep of this._deps) promises.push(dep());
			this._promise = Promise.all(promises).then(() => {
				logger.log(`All the dependincies of ${this.expr} have been ${chalk.green('resolved')}`);
